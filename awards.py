...
try:
    ...
    def graph_data(awd,name):
        data = [name,awd[0],awd[1],awd[2],awd[3]] 
        return data    
    ...
    with open('awards.csv', 'wb') as fp:
        a = csv.writer(fp, delimiter=',')
        a.writerow(['State','Oscar','Golden Globe', 'BAFTA','Emmy'])
        data = graph_data(a_awd_arr, 'Actor')
        a.writerow(data)

    with open('awards.csv', 'ab') as fp:
        a = csv.writer(fp, delimiter=',')
        data = graph_data(d_awd_arr , 'Director')
        a.writerow(data)

    with open('awards.csv', 'ab') as fp:
        a = csv.writer(fp, delimiter=',')
        data = graph_data(w_awd_arr , 'Writer')
        a.writerow(data)
